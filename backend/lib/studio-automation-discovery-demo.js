...
function buildStudioAutomationDiscoveryFollowupQuestionCardPayload({
	...
	return {
		...
		directive: "Use the recommended defaults to keep the demo focused on three rich agent drafts.",
		requiredCount: 3,
		questions: [
			{
				id: "creation-boundary",
				label: "Which candidates should become drafts now?",
				description: "This decides whether Studio creates only the strongest candidates or broadens the demo.",
				required: true,
				kind: "single-select",
...
const CREATED_AGENT_RESULTS = [
	{
		action: "create",
		agentId: "venn-loom-distribution-agent",
		name: "Loom Distribution Agent",
		byline: "Multi-channel distribution",
		sourceLabel: "Studio automation discovery demo",
		description: "Distributes your new design Looms across sprint channels, design stakeholders, and Atlas project updates with approval gates.",
		summary: "Summarizes new Loom sharebacks, drafts channel posts, prepares stakeholder FYIs, and queues Atlas updates for you to approve.",
		avatarSrc: "/avatar-agent/teamwork-agents/transcript-insights-reporter.svg",
		conversationStarters: [
			"Draft the shareback plan for my latest Agent builder Loom.",
			"Which stakeholders should get a personalized FYI for this Loom?",
			"Prepare an Atlas update from the latest Loom description.",
		],
		conversationStarterIcons: ["page", "people-group", "edit-bulk"],
		triggers: [
			"A new Loom video is published from your account",
			"A Loom title contains Agent builder, Studio, Rovo Skills, or Agent creation",
		],
		tools: ["loom", "slack", "confluence", "atlassian-home"],
		apps: ["loom", "slack", "confluence", "atlassian-home"],
		skills: ["summarize-thread", "build-report", "automation-rule-builder"],
		knowledge: ["Agents creation experience vision", "Studio v2 strategy check-in"],
		...
		instructions: [
			...
			"",
			"## Workflow",
			"1. Summarize the Loom in plain language.",
			"2. Identify the most relevant channels and stakeholder groups.",
			"3. Draft channel posts, personalized FYIs, and an Atlas update.",
			"4. Present everything for approval before posting.",
			"",
			"## Guardrails",
	...
	},
	{
		...
		sourceLabel: "Studio automation discovery demo",
		description: "Reviews agent-reaper-bot Jira tickets, distinguishes throwaway prototypes from active project agents, and summarizes only the decisions you need to make.",
		summary: "Triage inactive-agent cleanup batches by checking keep-list signals, closing obvious prototype tickets, and escalating active agents for review.",
		avatarSrc: "/avatar-agent/teamwork-agents/work-organizer.svg",
		conversationStarters: [
			"Review today's inactive-agent tickets and tell me what needs attention.",
			"Update my keep list from active Rovo Studio projects.",
			"Summarize which prototype agents can safely expire.",
		],
		...
		skills: ["jql-query-builder", "bulk-issue-editor", "automation-rule-builder"],
		knowledge: ["Active Rovo Studio projects", "Agent creation prototypes"],
		...
		instructions: [
			...
			"2. Compare each agent against active Studio, Rovo Skills, and Agent creation project signals.",
			"3. Auto-close only obvious prototype/test-agent tickets.",
			"4. Send me a concise Slack summary of kept, closed, and needs-review items.",
			"",
			"## Guardrails",
			"Never close an agent linked to an active project, template, customer-facing demo, or named stakeholder request.",
		].join("\n"),
	},
	{
		action: "create",
		...
		sourceLabel: "Studio automation discovery demo",
		description: "Synthesizes your weekly Looms, Confluence edits, Slack feedback, Atlas milestones, and meeting rhythms into sprint and Atlas update drafts.",
		summary: "Drafts weekly updates covering shipped work, decisions, feedback, blockers, and next focus for sprint channels and Atlas projects.",
		avatarSrc: "/avatar-agent/teamwork-agents/team-recap.svg",
		conversationStarters: [
			"Draft this week's Studio sprint update.",
			"Turn my latest Looms and Confluence edits into Atlas updates.",
			"What changed this week across Agent builder and Rovo Skills?",
		],
		...
		skills: ["standup-summarizer", "build-report", "milestone-tracker"],
		knowledge: ["Studio v2", "Rovo powered by Skills", "Agents creation experience vision"],
		...
		instructions: [
			...
			"2. Produce a compact update with sections for shipped work, decisions, feedback, blockers, and next focus.",
			"3. Draft sprint-channel copy and Atlas project update variants.",
			"4. Ask me to approve project status language before publishing.",
			"",
			"## Guardrails",
	...
	const agents = [
		{
			item: {
				...
				title: "Loom Distribution Agent",
				source: "Studio draft",
				owner: "Multi-channel distribution",
				logoSrc: "/avatar-agent/teamwork-agents/transcript-insights-reporter.svg",
			},
			agentResult: CREATED_AGENT_RESULTS[0],
		},
		{
			item: {
				id: "venn-inactive-agent-triage-agent",
				title: "Inactive Agent Triage Agent",
				source: "Studio draft",
				owner: "Lifecycle triage",
				logoSrc: "/avatar-agent/teamwork-agents/work-organizer.svg",
			},
			agentResult: CREATED_AGENT_RESULTS[1],
		},
		{
			item: {
				id: "venn-weekly-sprint-atlas-digest-agent",
				title: "Weekly Sprint/Atlas Digest Agent",
				source: "Studio draft",
				owner: "Weekly synthesis",
				logoSrc: "/avatar-agent/teamwork-agents/team-recap.svg",
			},
			agentResult: CREATED_AGENT_RESULTS[2],
		},
	];
