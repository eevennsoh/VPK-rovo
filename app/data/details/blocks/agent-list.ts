	...
	props: [
		{
			...
			description:
				"Rows to render. Each item's `state` (\"running\" | \"complete\" | \"needs-input\" | \"attention\") drives the status treatment and row actions; `attention` keeps the item's own title and shows a warning glyph, for notification rows whose title is already the news. `agent.kind` (\"agent\" | \"person\") picks the hexagon or circular avatar, so agents and teammates can share one list. Optional `summary` adds wrapping body copy below the metadata row, `metadataPrefix` a leading metadata chunk (e.g. `\"Risk · PAY-112\"`), and `timeLabel` a pre-formatted time in place of the live clock. Local rows (`host: \"local\"`) also take `machineName` and always show a static timestamp.",
		},
		{
			name: "flyout",
