...
export const OMNIBAR_DETAIL: ComponentDetail = {
	description:
		"A persistent bottom-center AI bar with three geometries. At rest it is a black lozenge holding the Rovo sparkle. Hovering cross-fades it into a compact prompt bar — `+`, editor, side-panel, and send — and pressing inside the bar pins it open so a draft survives the pointer leaving. The side-panel button docks a chat surface on the right. Pass `timelineEntries` and a Timeline context pill sits above the composer, scrubbing the Scrubber block's notch rail horizontally in the bar or vertically at the screen edge. Pill and bar are two independent surfaces that trade places on a shared z-axis rather than one box that resizes: each scales and fades in place, so nothing animates layout and the prompt text is never scaled by a parent. The expanded bar takes the light `PromptInput variant=\"floating\"` chrome by default; `tone=\"inverse\"` re-skins it onto `background.neutral.bold`, the same black the pill uses.",
