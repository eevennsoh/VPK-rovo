...
export const AGENT_SESSION_FLYOUT_DETAIL: ComponentDetail = {
	description:
		"The `/asx` queue session flyout, presented as one compact chat-history list. Every row feeds one shared, anchored flyout viewport, so hovering vertically between sessions moves and resizes the popup while its content crossfades. The flyout includes a title with a relative timestamp, an awaiting-response banner, Session/Agent/Work item rows (the agent as a Tag pill, the work item as a SmartLink with its status lozenge), and a separated \u201cDevelopment\u201d block for SCM fields. By default the block renders the four `/asx` sessions — awaiting user response, in progress, PR open, and PR merged.",
