...
async function loadBundledRoute(t) {
	const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "html-selector-tokens-route-"));
	t.after(() => {
...
test("GET /api/html-selector/tokens proxies to the backend token endpoint", async (t) => {
	const { GET } = await loadBundledRoute(t);
	const requests = mockBackendFetch(t, () => new Response(
		JSON.stringify({ color: { brand: "#0c66e4" } }),
		{ headers: { "Content-Type": "application/json" }, status: 200 },
	));

	const response = await GET(new Request("http://localhost/api/html-selector/tokens"));

	assert.equal(response.status, 200);
	assert.deepEqual(await response.json(), { color: { brand: "#0c66e4" } });
	assert.deepEqual(requests, [{
		method: "GET",
		url: "http://backend.local/api/html-selector/tokens",
	}]);
});
