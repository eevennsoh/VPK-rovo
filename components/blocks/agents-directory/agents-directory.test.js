...
test("Agent Directory includes Agent Templates as a sidebar mode", () => {
	...
	assert.match(source, /activeSetupAgentIds\.has\(agent\.id\) \? \([\s\S]*<ExperimentalTemplateSetupCard[\s\S]*agent=\{agent\}[\s\S]*onCancel=\{\(\) => handleCancelSetupAgent\(agent\.id\)\}[\s\S]*onBuildAgent=\{onBuildAgent\}[\s\S]*onOpenBuiltAgent=\{handleOpenBuiltAgent\}/u);
...
test("Agent Directory experimental templates use the setup flow before opening config", () => {
	...
	assert.match(source, /import \{ GreetingPromptRow \} from "@\/components\/projects\/shared\/components\/greeting-prompt-row";/u);
	assert.match(source, /import \{ AnimatePresence, cubicBezier, motion, useReducedMotion \} from "motion\/react";/u);
	assert.match(source, /import \{ ProgressTracker, type ProgressTrackerStep \} from "@\/components\/ui\/progress-tracker";/u);
	assert.match(source, /import \{ getAppById, type DirectoryApp \} from "@\/app\/data\/directory\/apps";/u);
	assert.match(source, /import \{ AtlassianLogoMark, BrandLogoMark \} from "@\/components\/ui\/logo-mark";/u);
	...
	assert.match(source, /<GreetingPromptRow[\s\S]*className="group\/template-app-row shrink-0"[\s\S]*onClick=\{\(\) => handleToggleApp\(source\.id\)\}[\s\S]*selected=\{isSelected\}/u);
	...
	assert.doesNotMatch(source, /hiddenAppCount/u);
	assert.doesNotMatch(source, /\+\{hiddenAppCount\} more/u);
	assert.doesNotMatch(source, /AGENT_BROWSER_TEMPLATE_THIRD_PARTY_PROVIDER_LOGOS/u);
	assert.match(source, /function getExperimentalTemplateSourceLogo\([\s\S]*source: NonNullable<AgentTemplatesAgent\["sources"\]>\[number\],[\s\S]*\) \{[\s\S]*const directoryApp = getAppById\(source\.id\);[\s\S]*if \(directoryApp\) \{[\s\S]*return getExperimentalTemplateDirectoryAppLogo\(directoryApp\);[\s\S]*if \(source\.iconSrc\) \{[\s\S]*return <BrandLogoMark frame="tile" label=\{source\.label\} size="medium" src=\{source\.iconSrc\} \/>;[\s\S]*if \(source\.provider === "google-drive" \|\| source\.provider === "salesforce"\) \{[\s\S]*<BrandLogoMark[\s\S]*src=\{`\/3p\/\$\{source\.provider\}\/24\.svg`\}[\s\S]*return <AtlassianLogoMark label=\{source\.label\} name=\{source\.provider as AtlassianLogoName\} size="medium" \/>;/u);
	assert.match(source, /function getExperimentalTemplateDirectoryAppLogo\(app: DirectoryApp\) \{[\s\S]*if \(app\.logoName \|\| app\.id === "atlassian"\) \{[\s\S]*return <AtlassianLogoMark label=\{app\.name\} name=\{app\.logoName \?\? "atlassian"\} size="medium" \/>;[\s\S]*const src = app\.logoSrc \?\? app\.avatarSrc;[\s\S]*return src \? <BrandLogoMark frame="tile" label=\{app\.name\} size="medium" src=\{src\} \/> : null;/u);
	assert.match(source, /<GreetingPromptRow[\s\S]*description=\{`Use \$\{source\.label\} context while setting up \$\{agent\.name\}\.`\}[\s\S]*visual=\{getExperimentalTemplateSourceLogo\(source\)\}/u);
