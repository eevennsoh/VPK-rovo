...
test("the Agents section directly reuses Jira Agent Session with native states and selection", () => {
	assert.match(
		DETAIL_PANEL_SOURCE,
		/import \{[\s\S]*JiraAgentSession,[\s\S]*type JiraAgentSessionItem,[\s\S]*\} from "@\/components\/blocks\/jira-agent-session";/u,
	);
	...
	assert.match(DETAIL_PANEL_SOURCE, /<JiraAgentSession[\s\S]*className="w-full"/u);
