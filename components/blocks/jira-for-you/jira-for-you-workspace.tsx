...
import { JiraForYouAgentSessionsWorkspace } from "./jira-for-you-agent-sessions-workspace";
...
const WIDE_AGENT_SESSIONS_PREFERRED_WIDTH_PX = 1200;
...
export function JiraForYouWorkspace({
	...
	const wideChatWorkspaceStyle = !isNarrow
		...
		: undefined;
	const wideAgentSessionsWorkspaceStyle = !isNarrow
		? {
			flexBasis: WIDE_AGENT_SESSIONS_PREFERRED_WIDTH_PX,
			maxWidth: `calc(100% - ${WIDE_FEED_MIN_WIDTH_PX}px)`,
		}
		: undefined;
		...
		>
			...
			<AnimatePresence initial={false}>
				...
				unassignedItemData ? (
					<motion.section
						animate="open"
						aria-label={`Agent Sessions workspace for ${unassignedItemData.item.issueKey}`}
						className={cn(
							"min-h-0 min-w-0 bg-background",
							isNarrow
								? "absolute inset-0 z-10"
						...
						data-layout={isNarrow ? "overlay" : "split"}
						data-testid="jira-for-you-agent-sessions-container"
						exit="closed"
						initial="closed"
						style={{
							...wideAgentSessionsWorkspaceStyle,
							willChange: isNarrow && !shouldReduceMotion ? "transform" : undefined,
						}}
						variants={workspaceVariants}
					>
						<JiraForYouAgentSessionsWorkspace
							onBack={handleCloseWorkspace}
