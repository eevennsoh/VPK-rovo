...
test("collapsed markers default to circles and can retain the previous line treatment", () => {
	...
	assert.match(RAIL_COLUMN_SOURCE, /notchShape = "circle",/u);
	assert.match(RAIL_COLUMN_SOURCE, /notchShape === "line"\s*\? \(\s*<AgentSessionNotchMark/u);
	assert.match(RAIL_COLUMN_SOURCE, /:\s*\(\s*<AgentSessionUserNotch/u);
	assert.match(
		RAIL_COLUMN_SOURCE,
