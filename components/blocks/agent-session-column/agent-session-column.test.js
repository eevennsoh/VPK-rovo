...
test("the column is a sunken surface, unlike the board's unfilled status columns", () => {
	assert.match(
		INDEX_SOURCE,
		/const AGENT_SESSION_PLANE = "[^"]*bg-surface-sunken p-1[^"]*";/u,
	);
	// Sunken is the whole point of the column — not the chin, not `bg-surface`,
	// not the uncaptured-work card grey.
	assert.doesNotMatch(INDEX_SOURCE, /const AGENT_SESSION_PLANE = "[^"]*bg-surface p-1/u);
	assert.doesNotMatch(
		INDEX_SOURCE,
		/const AGENT_SESSION_PLANE = "[^"]*bg-bg-accent-gray-subtlest/u,
	);
	assert.match(DETAIL_SOURCE, /filled with `surface-sunken`/u);
	assert.doesNotMatch(DETAIL_SOURCE, /bg-bg-accent-gray-subtlest|grey well/u);
	// The fill is owned by the column, so it must not be reachable only
...
test("the session plane matches the flyout list parent's padding, not its gap", () => {
	...
	// keeps that inset, but large uncaptured cards stack as one dashed well.
	assert.match(INDEX_SOURCE, /const AGENT_SESSION_PLANE = "[^"]*p-1[^"]*";/u);
...
test("the fill starts below the header, so the title shares the status columns' baseline", () => {
	...
	assert.doesNotMatch(
		INDEX_SOURCE,
		/<section[\s\S]*?className=\{cn\(\s*"[^"]*bg-surface-sunken/u,
	);
...
test("each notch opens the shared session flyout rather than a forked preview", () => {
	...
	assert.match(RAIL_COLUMN_SOURCE, /render=\{<div className="h-5 w-full" \/>\}/u);
	assert.doesNotMatch(RAIL_COLUMN_SOURCE, /<JiraSessionFlyoutTrigger[\s\S]*render=\{\s*<motion\.li/u);
	assert.match(RAIL_COLUMN_SOURCE, /<motion\.li[\s\S]*<JiraSessionFlyoutTrigger/u);
