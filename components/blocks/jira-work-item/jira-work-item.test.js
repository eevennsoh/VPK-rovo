...
test("JiraWorkItem exposes the variant API and the minimal experimental preset prop", () => {
	assert.match(JIRA_WORK_ITEM_SOURCE, /export type JiraWorkItemVariant = "default" \| "experimental" \| "experimental-v2" \| "experimental-v3" \| "experimental-v4";/u);
	...
	assert.match(
		JIRA_WORK_ITEM_SOURCE,
		/const EXPERIMENTAL_SURFACES = \{\s*experimental: ExperimentalJiraWorkItem,\s*"experimental-v2": ExperimentalV2JiraWorkItem,\s*"experimental-v3": ExperimentalV3JiraWorkItem,\s*"experimental-v4": ExperimentalV4JiraWorkItem,\s*\} as const;/u,
	);
...
test("page.tsx is the 4-button hero chooser (standard + experimental v1/v2/v3, all filled) that remounts the block", () => {
	...
	assert.equal((pageSource.match(/variant="outline"/gu) ?? []).length, 4);
	assert.match(
		pageSource,
		/<Button type="button" onClick=\{\(\) => setActiveVariant\("experimental-v4"\)\}>/u,
	);
...
test("JiraWorkItem keeps the standard + experimental registry, detail, demo, and preview wiring", () => {
	...
	assert.match(detailsSource, /name: "variant"[\s\S]*type: "\\"default\\" \| \\"experimental\\" \| \\"experimental-v2\\" \| \\"experimental-v3\\" \| \\"experimental-v4\\"/u);
