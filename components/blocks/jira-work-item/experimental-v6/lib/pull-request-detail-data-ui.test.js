...
test("detail UI exposes stable integration selectors and guided-review controls", () => {
	...
	const workItemSource = readFileSync(
		join(__dirname, "../experimental-v6-jira-work-item.tsx"),
		"utf8",
	);
	...
	assert.match(
		overviewSource,
		/import \{ ContextDescriptionEditor \} from "@\/components\/blocks\/jira-work-item\/experimental-v6\/components\/context-description-editor"/u,
	);
	assert.match(
		overviewSource,
	...
	assert.match(
		scrollSpyHookSource,
		/from "@\/components\/blocks\/jira-work-item\/experimental-v6\/lib\/pull-request-guide-active-chapter"/u,
	);
	assert.match(
		scrollSpyHookSource,
