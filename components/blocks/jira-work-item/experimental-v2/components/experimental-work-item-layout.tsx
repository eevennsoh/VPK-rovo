 ...
 * becomes `sticky bottom-0` on a translucent, blurred dock so it stays visible
 * at the bottom of the single scroll. That outer scrollport owns a matching
 * progressive bottom mask while more content remains below. The same order
 * values also keep each wide column stacked correctly, so no slot is rendered twice.
 * The wide Context scrollport keeps its own top and bottom masks.
 *
...
export function ExperimentalWorkItemLayout({
	...
	return (
		<div className="@container/agentlayout h-full min-h-0 min-w-0">
			<div
				ref={narrowScrollRef}
				className="flex h-full min-h-0 min-w-0 flex-col gap-6 overflow-y-auto p-6 data-[fill-container]:pb-0 @[860px]/agentlayout:relative @[860px]/agentlayout:grid @[860px]/agentlayout:grid-cols-1 @[860px]/agentlayout:grid-rows-[auto_minmax(0,1fr)] @[860px]/agentlayout:gap-0 @[860px]/agentlayout:overflow-hidden @[860px]/agentlayout:p-0"
