...
// Contract tests for the `experimental-v5` fork of the Jira Work Item surface.
//
...
const V3_DIR = path.join(BLOCK_DIR, "experimental-v5");
...
test("experimental v5 exists with a distinctly named composition root", () => {
	assert.ok(fs.existsSync(V3_DIR));

	const compositionSource = readBlockFile("experimental-v5/experimental-v5-jira-work-item.tsx");
	assert.match(compositionSource, /export function ExperimentalV5JiraWorkItem\(/u);
	assert.match(compositionSource, /export default ExperimentalV5JiraWorkItem;/u);
	assert.match(compositionSource, /export type ExperimentalV5JiraWorkItemProps/u);
...
test("experimental v5 is isolated from v1 and v2", () => {
	...
	for (const olderDir of [V1_DIR, V2_DIR]) {
		for (const filePath of listSourceFiles(olderDir)) {
			assert.doesNotMatch(
				...
				/experimental-v5|ExperimentalV5/u,
...
test("the v5 surface has one section nav, not a rail toggle plus a PR tab strip", () => {
	...
	const navSource = readBlockFile("experimental-v5/components/work-item-section-nav.tsx");
	...
	const detailViewSource = readBlockFile(
		"experimental-v5/components/pull-request-detail/pull-request-detail-view.tsx",
	);
	assert.doesNotMatch(detailViewSource, /<TabsList|<TabsContent|tabNavigation/u);
});

test("the v5 control row is consolidated and Reporter moved back to Details", () => {
	const resourcesSource = readBlockFile("experimental-v5/components/context-resources.tsx");
	assert.match(
		resourcesSource,
	...
	assert.match(
		readBlockFile("experimental-v5/components/pull-requests-select.tsx"),
		/span[\s\S]*className="shrink-0 text-xs font-normal text-text-subtlest"[\s\S]*\{pullRequestCount\}/u,
	);

	const detailsTabSource = readBlockFile("experimental-v5/components/details-tab.tsx");
	assert.match(
		detailsTabSource,
	...
	const detailsEditorsSource = readBlockFile("experimental-v5/components/detail-field-editors.tsx");
	assert.match(
	...
	const composerAgentPillSource = readBlockFile("experimental-v5/components/activity-composer-agent-context-pill.tsx");
	assert.match(composerAgentPillSource, /import \{ WORK_ITEM_AGENT_SELECTOR_MENU \} from "@\/components\/blocks\/jira-work-item\/experimental-v5\/lib\/work-item-agent-selector-menu";/u);
	assert.match(
	...
	assert.match(
		detailsTabSource,
		/import \{[\s\S]*AgentsRowField,[\s\S]*\} from "@\/components\/blocks\/jira-work-item\/experimental-v5\/components\/detail-field-editors";/u,
	);
	assert.match(
		detailsTabSource,
	...
	const titleBarSource = readBlockFile("experimental-v5/components/context-title-bar.tsx");
	...
	const titleTreatmentSource = readBlockFile(
		"experimental-v5/components/inline-edit-treatment.ts",
	);
	assert.match(
	...
	assert.match(
		readBlockFile("experimental-v5/components/context-editable-header.tsx"),
		/font-medium!/u,
	);
	...
	const resourcesSourceWithCompactState = readBlockFile(
		"experimental-v5/components/context-resources.tsx",
	);
	assert.match(
		resourcesSourceWithCompactState,
		/compact \? null : \([\s\S]*<ContextTitleActions[\s\S]*selectedAgentId=\{selectedAgentId\}/u,
	);
	const titleActionsSource = readBlockFile("experimental-v5/components/context-title-actions.tsx");
...
test("the v5 dialog owns one fixed chrome row", () => {
	const dialogSource = readBlockFile("experimental-v5/components/experimental-work-item-dialog.tsx");
	...
	const layoutSource = readBlockFile("experimental-v5/components/experimental-work-item-layout.tsx");
	...
	const contextPanelSource = readBlockFile("experimental-v5/components/context-panel.tsx");
	assert.doesNotMatch(contextPanelSource, /ContextHeader|data-jira-work-item-context-header|WorkItemSectionNav/u);

	const spyHookSource = readBlockFile("experimental-v5/hooks/use-scroll-spy-sections.ts");
	assert.match(
		spyHookSource,
	...
	const navigationSource = readBlockFile("experimental-v5/context-section-navigation.tsx");
	...
	assert.match(
		readBlockFile("experimental-v5/experimental-v5-jira-work-item.tsx"),
		/<SectionNavigationProvider active=\{open\}>/u,
	);

	const stickyShellSource = readBlockFile(
		"experimental-v5/components/pull-request-detail/pull-request-sticky-header-shell.tsx",
	);
	...
	assert.match(
		readBlockFile("experimental-v5/lib/work-item-section-tabs.ts"),
...
test("v5 header collapse control stays static and does not toggle header mode", () => {
	...
	const headerActionsSource = readBlockFile(
		"experimental-v5/components/experimental-breadcrumb-actions.tsx",
	);
...
test("PR select shares the section navigation list without becoming a section", () => {
	...
	const selectSource = readBlockFile("experimental-v5/components/pull-requests-select.tsx");
	...
	const sectionTabsSource = readBlockFile("experimental-v5/lib/work-item-section-tabs.ts");
	assert.match(
		sectionTabsSource,
	...
	assert.doesNotMatch(
		readBlockFile("experimental-v5/components/work-item-body.tsx"),
		/id="insights"/u,
	);
	assert.match(
		readBlockFile("experimental-v5/components/context-panel.tsx"),
	...
	const sectionNavigationSource = readBlockFile("experimental-v5/context-section-navigation.tsx");
	assert.match(
	...
	assert.match(
		readBlockFile("experimental-v5/components/insights-panel.tsx"),
	...
	const contextPillsSource = readBlockFile("experimental-v5/components/activity-composer-context-pills.tsx");
...
test("experimental v5 insight sources reuse work-item, session, activity, and pull-request owners", () => {
	const compositionSource = readBlockFile("experimental-v5/experimental-v5-jira-work-item.tsx");
	const composerSource = readBlockFile("experimental-v5/components/activity-composer.tsx");
	const pillsSource = readBlockFile("experimental-v5/components/activity-composer-context-pills.tsx");
...
test("experimental v5 shares one insight selection between the filtered feed and editorial rail", () => {
	const metadataSource = readBlockFile("experimental-v5/components/metadata-rail.tsx");
	const contextSource = readBlockFile("experimental-v5/components/context-panel.tsx");
	...
	assert.match(
		readBlockFile("experimental-v5/components/insights-work-item-split.tsx"),
	...
	assert.match(
		readBlockFile("experimental-v5/components/work-item-side-panel-resize-handle.tsx"),
		/SidebarResizeHandle/u,
	);
});

test("experimental v5 does not render a closed-state floating Rovo launcher", () => {
	const sessionSurfaceSource = readBlockFile("experimental-v5/components/floating-session-surface.tsx");
...
test("experimental v5 shares the session/planner data layer", () => {
	assert.match(
		readBlockFile("experimental-v5/experimental-v5-jira-work-item.tsx"),
		/@\/components\/blocks\/jira-work-item\/data\/session-state/u,
	);
	assert.match(
		readBlockFile("experimental-v5/use-jira-work-item-controller.ts"),
		/@\/components\/blocks\/jira-work-item\/data\/session-state/u,
	);
	// No forked copy of the model lives under v5.
...
test("Activity sort hover-reveals from the section group, not a local copy", () => {
	const sectionSource = readBlockFile("experimental-v5/components/work-item-section.tsx");
	const activityPanelSource = readBlockFile("experimental-v5/components/activity-panel.tsx");
...
test("experimental v5 uses one chronological Activity feed for activity and insights", () => {
	const activityPanelSource = readBlockFile("experimental-v5/components/activity-panel.tsx");
...
test("experimental v5 header Actions menu copies the work item as markdown", () => {
	const overflowMenuSource = readBlockFile(
		"experimental-v5/components/experimental-header-overflow-menu.tsx",
	);
	const descriptionSource = readBlockFile(
		"experimental-v5/components/context-editable-header.tsx",
	);
	...
	assert.doesNotMatch(
		readBlockFile("experimental-v5/components/context-resources.tsx"),
...
test("the v5 assigned-agents menu lists live agent state and ends in an Assign agent row", () => {
	...
	assert.match(
		detailsEditorsSource,
		/import \{ WorkingSessionActivityByline \} from "@\/components\/blocks\/jira-work-item\/experimental-v5\/components\/agent-session-activity-byline";/u,
	);
	assert.match(
		menuSource,
...
test("the v5 working-session byline is one shared module, not a per-surface copy", () => {
	const bylineSource = readBlockFile("experimental-v5/components/agent-session-activity-byline.tsx");
	const contextPillsSource = readBlockFile("experimental-v5/components/activity-composer-context-pills.tsx");
	...
	assert.match(
		contextPillsSource,
		/import \{ NEEDS_INPUT_STATUS_LABEL, WorkingSessionActivityByline \} from "@\/components\/blocks\/jira-work-item\/experimental-v5\/components\/agent-session-activity-byline";/u,
	);
...
test("the v5 lib test suite is registered so it actually runs in CI", () => {
	...
	for (const testFile of registeredV2Tests) {
		assert.match(
			manifestSource,
			new RegExp(`"components/blocks/jira-work-item/experimental-v5/lib/${testFile}"`, "u"),
	...
	assert.match(
		manifestSource,
		/"components\/blocks\/jira-work-item\/experimental-v5\/lib\/assigned-agent-rows\.test\.js"/u,
	);
});
