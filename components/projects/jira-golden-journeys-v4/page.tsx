...
import { JIRA_GOLDEN_JOURNEYS_V4_GALLERY_ITEMS } from "./data/gallery-items";
import {
	...
	type JiraGoldenJourneysV4PresentationChapter,
...
import {
	JiraGoldenJourneysV4CompactStoryControls,
	JiraGoldenJourneysV4StoryControls,
} from "./story-controls";
import { JiraGoldenJourneysV4TerminalStory } from "./terminal-story";
...
function JiraGoldenJourneysV4TrackLearnStage({
	boardColumns,
	boardRef,
	chapter,
	...
	onChapterChange: (chapter: JiraGoldenJourneysV4PresentationChapter) => void;
	...
	return (
		<div className="relative left-1/2 flex h-full min-h-0 w-[100cqw] -translate-x-1/2 flex-col overflow-hidden pt-4 [&>div]:h-full [&>div]:min-h-0">
			<ExperimentalJiraKanbanPage
				...
				isLooseWorkResumable={(item) => item.id === JIRA_GOLDEN_JOURNEYS_V4_PAY_101_UNCAPTURED_SESSION_ID}
...
function JiraGoldenJourneysV4BuildStage({
	revision,
}: Readonly<{ revision: number }>): React.ReactElement {
	const initialState = useMemo(() => createJiraGoldenJourneysV4Pay101BuildState(), []);
	...
	return (
		<div className="relative left-1/2 flex h-full min-h-0 w-[100cqw] -translate-x-1/2 items-start justify-center overflow-hidden px-8 pt-4 pb-4">
			<ExperimentalV4JiraWorkItem
				activitySessionThread={{
					parentSessionId: JIRA_GOLDEN_JOURNEYS_V4_PAY_101_SESSION_ID,
					childSessionIds: [],
					visibleSessionIds: [JIRA_GOLDEN_JOURNEYS_V4_PAY_101_SESSION_ID],
					autoScroll: false,
					defaultRepliesExpanded: false,
				}}
				composerAgents={JIRA_GOLDEN_JOURNEYS_V4_PAY_COMPOSER_AGENTS}
				...
				statusPhases={JIRA_GOLDEN_JOURNEYS_V4_PAY_STATUS_PHASES}
				workItem={JIRA_GOLDEN_JOURNEYS_V4_PAY_101_WORK_ITEM}
			/>
		</div>
	);
}
...
function JiraGoldenJourneysV4App(): React.ReactElement {
	...
	const [selectedId, setSelectedId] = useState(JIRA_GOLDEN_JOURNEYS_V4_GALLERY_ITEMS[0]?.id ?? "");
	const [chapter, setChapter] = useState<JiraGoldenJourneysV4PresentationChapter>("track");
	...
	const terminalController = useTerminalDemo(
		...
		{ story: JIRA_GOLDEN_JOURNEYS_V4_TERMINAL_STORY },
	);
	const terminalStep = Math.min(
		terminalController.state.beatIndex + 2,
		JIRA_GOLDEN_JOURNEYS_V4_TERMINAL_STEP_COUNT,
	);
	const handleChapterChange = useCallback((nextChapter: JiraGoldenJourneysV4PresentationChapter) => {
	...
	const resetStory = useCallback(() => {
		setChapter("track");
		setStageRevision((current) => current + 1);
		setBoardColumns(createJiraGoldenJourneysV4PayBoardColumns());
	...
	const handleResumeLooseWork = useCallback(async (item: PulseLooseWork) => {
		if (
			...
			|| item.id !== JIRA_GOLDEN_JOURNEYS_V4_PAY_101_UNCAPTURED_SESSION_ID
		) return;
		try {
			await navigator.clipboard.writeText(JIRA_GOLDEN_JOURNEYS_V4_RESUME_PROMPT);
	...
	const renderSelectedItem = useCallback((item: GalleryItem): React.ReactNode => {
		...
		if (chapter === "terminal") {
			return (
				<JiraGoldenJourneysV4TerminalStory
		...
		if (chapter === "build") {
			return <JiraGoldenJourneysV4BuildStage revision={stageRevision} />;
		}
		return (
			<JiraGoldenJourneysV4TrackLearnStage
				boardColumns={boardColumns}
				boardRef={boardRef}
				chapter={chapter}
	...
	return (
		<>
			...
			>
				...
				<Gallery
					items={JIRA_GOLDEN_JOURNEYS_V4_GALLERY_ITEMS}
					onReset={resetStory}
					title="Jira Golden Journeys v4"
