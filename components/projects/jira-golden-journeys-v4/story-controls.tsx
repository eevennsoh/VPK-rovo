...
import {
	JIRA_GOLDEN_JOURNEYS_V4_PRESENTATION_CHAPTERS,
	type JiraGoldenJourneysV4PresentationChapter,
} from "./data/presentation-story";
import { JIRA_GOLDEN_JOURNEYS_V4_TERMINAL_STEP_COUNT } from "./data/terminal-story";

export function JiraGoldenJourneysV4StoryControls({
	chapter,
	onChapterChange,
	terminalStep,
}: Readonly<{
	chapter: JiraGoldenJourneysV4PresentationChapter;
	...
	return (
		...
		>
			<ButtonGroup
				...
				variant="connected"
			>
				{JIRA_GOLDEN_JOURNEYS_V4_PRESENTATION_CHAPTERS.map((option) => (
					<Button
					...
					>
						{option.value === "terminal"
							? `${option.label} · ${terminalStep} of ${JIRA_GOLDEN_JOURNEYS_V4_TERMINAL_STEP_COUNT}`
							: option.label}
					</Button>
				))}
			</ButtonGroup>
...
export function JiraGoldenJourneysV4CompactStoryControls({
	chapter,
	onChapterChange,
}: Readonly<{
	chapter: JiraGoldenJourneysV4PresentationChapter;
	...
	const activeIndex = JIRA_GOLDEN_JOURNEYS_V4_PRESENTATION_CHAPTERS.findIndex(
		(option) => option.value === chapter,
	);
	const activeChapter = JIRA_GOLDEN_JOURNEYS_V4_PRESENTATION_CHAPTERS[activeIndex];
	const previousChapter = JIRA_GOLDEN_JOURNEYS_V4_PRESENTATION_CHAPTERS[activeIndex - 1];
	const nextChapter = JIRA_GOLDEN_JOURNEYS_V4_PRESENTATION_CHAPTERS[activeIndex + 1];
