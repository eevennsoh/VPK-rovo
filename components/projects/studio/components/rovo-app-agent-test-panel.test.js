...
test("AgentTestPanel isolates the live-draft agent in a nested chat provider", () => {
	...
	assert.match(AGENT_TEST_PANEL_SOURCE, /const draftSnapshotKey = JSON\.stringify\(entry\.draftResult\);/u);
	assert.match(AGENT_TEST_PANEL_SOURCE, /const snapshotKey = `\$\{entry\.profile\.id\}:\$\{draftSnapshotKey\}`;/u);
...
test("AgentTestPanel builds test profile data from the live draft", () => {
	assert.match(AGENT_TEST_PANEL_SOURCE, /const payload = entry\.draftResult as AgentResultPayload;/u);
