...
test("Jira Agents composes the six-chapter software delivery story without changing the gallery", () => {
	...
	assert.equal((itemsSource.match(/\bid:\s*"/gu) ?? []).length, 2);
	assert.match(itemsSource, /id: "work-item",[\s\S]*id: "kanban-list"/u);
...
test("Jira Agents seeds checkout automation rows without changing the shared empty default", () => {
	...
	assert.match(compositionSource, /<MetadataRail automationRules=\{props\.automationRules\} borderless \/>/u);
...
test("the software delivery chapters preserve the scripted status and agent-working progression", async () => {
	...
	assert.deepEqual(chapters, ["brief", "plan", "working", "handoff", "review", "done"]);
	assert.deepEqual(
		story.JIRA_AGENTS_STORY_COMPOSER_AGENTS.map((agent) => agent.name),
		["Code Planner", "GitHub Copilot", "Unit Test Creator"],
	);
	assert.deepEqual(
		chapters.map((chapter) => story.getJiraAgentsStoryStatus(chapter)),
		["To do", "In progress", "In progress", "In progress", "In review", "Done"],
	);
	assert.deepEqual(
...
test("working, handoff, review, and done encode the agent dependency chain", async () => {
	const story = await loadStoryModule();
	...
	const workingCopilot = working.sessions.find((session) => session.agentId === "github-copilot");
	const handoffCopilot = handoff.sessions.find((session) => session.agentId === "github-copilot");
	const handoffTests = handoff.sessions.find((session) => session.agentId === "unit-test-creator");
	const reviewTests = review.sessions.find((session) => session.agentId === "unit-test-creator");

	assert.equal(workingCopilot.status, "waiting");
	assert.deepEqual(workingCopilot.waitingOn, {
		kind: "agent",
		agentId: "code-planner",
		agentName: "Code Planner",
		agentAvatarSrc: "/avatar-agent/dev-agents/code-planner.svg",
	});
	assert.equal(handoffCopilot.status, "running");
	assert.equal(handoffCopilot.threadReplies[0].agentId, "code-planner");
	assert.equal(handoffTests.status, "waiting");
	assert.equal(handoffTests.waitingOn.agentId, "github-copilot");
	assert.equal(reviewTests.status, "running");
	assert.equal(reviewTests.threadReplies[0].agentId, "github-copilot");
	...
	assert.ok(done.staticEvents.some((event) => event.id === "story-release-verified"));
...
test("the authored artifact and pull-request chronology is complete and renderable", async () => {
	...
	assert.deepEqual(
			...
			.filter((event) => [
				"story-telemetry",
				"story-root-cause-handoff",
				"story-changed-files",
				"story-pr-opened",
				"story-regression-matrix",
				"story-pr-merged",
				"story-release-verified",
			].includes(event.id))
			.map((event) => event.id),
		[
			"story-telemetry",
	...
	assert.deepEqual(
		...
		[
			"Guest checkout technical plan",
			"Guest checkout OpenAPI contract",
			"Checkout validation rules",
			"Guest checkout API contract",
			"Guest checkout implementation",
			"SHOP-4821 acceptance report",
			"Guest checkout rollout note",
		],
	);
});
...
test("the orchestration timeline and live broadcasts remain chronological", async () => {
	...
	const telemetry = working.staticEvents.find((event) => event.id === "story-telemetry");
	...
	assert.ok(leadClaim.createdAtMs < codePlanner.startedAtMs);
	assert.ok(codePlanner.startedAtMs < telemetry.createdAtMs);
	...
	assert.equal(
		broadcast.sessions.find((session) => session.agentId === "github-copilot").status,
		"waiting",
	);
});
