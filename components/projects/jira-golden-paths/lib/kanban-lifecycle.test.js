...
test("JGP board starts with no Drafting cards and derives every count", async () => {
	const { createInitialJgpKanbanState } = await loadHarness();
	const state = createInitialJgpKanbanState();
...
test("quick agent assignment moves an Intake card into Drafting and supports multiple agents", async () => {
	...
	state = jgpKanbanReducer(state, { type: "assign-agent", cardCodes: ["RFP-102"], agent: { id: "rfp-drafter" } });
	...
	state = jgpKanbanReducer(state, { type: "assign-agent", cardCodes: ["RFP-102"], agent: { id: "dependency-mapper" } });
	const presentedCard = resolveJgpKanbanColumns(state)
...
test("skill and custom-agent actions resolve to distinct activity rows", async () => {
	const { createJgpKanbanActivity, getJgpGenerativeAgentSelection, getJgpGenerativeActivityId } = await loadHarness();
	const skillRequest = {
		kind: "skill",
	...
	const skillActivity = createJgpKanbanActivity(getJgpGenerativeActivityId(skillRequest));
	const agentSelection = getJgpGenerativeAgentSelection(agentRequest);
	const agentActivity = createJgpKanbanActivity(agentSelection.id, false, agentSelection);
...
test("agent and skill selections produce distinct thinking, flyout, and unblock copy", async () => {
	const { createJgpKanbanActivity } = await loadHarness();
	const aiInsights = createJgpKanbanActivity("ai-insights-agent", true);
	const readiness = createJgpKanbanActivity("readiness-checker", true);
	const landingPage = createJgpKanbanActivity("skill:design-landing-page", true);
	const mobileInterface = createJgpKanbanActivity("skill:develop-mobile-app-interface", true);
...
test("multi-selected Intake cards batch-start when dropped into Drafting", async () => {
	...
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-102", columnTitle: "RFP Intake", indexInColumn: 1, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-103", columnTitle: "RFP Intake", indexInColumn: 2, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "drag-start", cardCode: "RFP-102", sourceColumnTitle: "RFP Intake" });
	state = jgpKanbanReducer(state, { type: "drop", targetColumnTitle: "Drafting" });
...
test("RFP-101 resolves needs-input and completion states before moving to Review", async () => {
	const { jgpKanbanReducer, createInitialJgpKanbanState, resolveJgpKanbanColumns } = await loadHarness();
	let state = createInitialJgpKanbanState();
	state = jgpKanbanReducer(state, { type: "assign-agent", cardCodes: ["RFP-101"], agent: { id: "rfp-drafter" } });
	state = jgpKanbanReducer(state, { type: "advance-generating", cardCode: "RFP-101" });
	assert.equal(state.lifecycleByCode["RFP-101"].phase, "generating");

	state = jgpKanbanReducer(state, { type: "request-input", cardCode: "RFP-101" });
	let presentedCard = resolveJgpKanbanColumns(state)
		.find((candidate) => candidate.title === "Drafting").cards[0];
	...
	state = jgpKanbanReducer(state, { type: "answer-question", cardCode: "RFP-101" });
	assert.equal(state.lifecycleByCode["RFP-101"].phase, "generating");
	state = jgpKanbanReducer(state, { type: "complete", cardCode: "RFP-101", generatedOutput: "Draft ready" });
	presentedCard = resolveJgpKanbanColumns(state)
...
test("completed runs preserve agent-specific summary and selected avatar", async () => {
	...
	state = jgpKanbanReducer(state, { type: "complete", cardCode: "RFP-102" });
	const completedRun = resolveJgpKanbanColumns(state)
...
test("toolbar set-status moves all selected cards to the chosen column", async () => {
	...
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-102", columnTitle: "RFP Intake", indexInColumn: 1, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-103", columnTitle: "RFP Intake", indexInColumn: 2, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "set-status", targetColumnTitle: "Review" });
...
test("toolbar assign/unassign toggles agents across selected cards", async () => {
	...
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-102", columnTitle: "RFP Intake", indexInColumn: 1, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "assign-toolbar-agent", agent: { id: "rfp-drafter" }, assigned: true });
	assert.deepEqual(state.lifecycleByCode["RFP-102"].agentIds, ["rfp-drafter"]);

	state = jgpKanbanReducer(state, { type: "assign-toolbar-agent", agent: { id: "rfp-drafter" }, assigned: false });
...
test("toolbar clear-selection empties the selection", async () => {
	...
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-102", columnTitle: "RFP Intake", indexInColumn: 1, modifiers: { shiftKey: false, metaOrCtrlKey: false } });
...
test("bulk-assigning one agent produces varied, non-lockstep working states per card", async () => {
	...
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-102", columnTitle: "RFP Intake", indexInColumn: 1, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-103", columnTitle: "RFP Intake", indexInColumn: 2, modifiers: toggle });
	state = jgpKanbanReducer(state, { type: "select", cardCode: "RFP-104", columnTitle: "RFP Intake", indexInColumn: 3, modifiers: toggle });
	state = jgpKanbanReducer(state, {
		type: "assign-agent",
	...
	const draftingCards = resolveJgpKanbanColumns(state)
	...
	const rerender = ["RFP-102", "RFP-103", "RFP-104"].map((code) =>
		resolveJgpKanbanColumns(state).find((candidate) => candidate.title === "Drafting").cards
...
test("drops outside Intake to Drafting do not start a lifecycle", async () => {
	const { jgpKanbanReducer, createInitialJgpKanbanState } = await loadHarness();
	let state = createInitialJgpKanbanState();
	state = jgpKanbanReducer(state, { type: "drag-start", cardCode: "RFP-161", sourceColumnTitle: "Review" });
	state = jgpKanbanReducer(state, { type: "drop", targetColumnTitle: "Drafting" });
...
test("dropping a completed card into Submitted moves it and clears the Agent done footer", async () => {
	...
	state = jgpKanbanReducer(state, { type: "assign-agent", cardCodes: ["RFP-102"], agent: { id: "rfp-drafter" } });
	state = jgpKanbanReducer(state, { type: "complete", cardCode: "RFP-102" });
	const reviewCard = resolveJgpKanbanColumns(state)
	...
	state = jgpKanbanReducer(state, { type: "drag-start", cardCode: "RFP-102", sourceColumnTitle: "Review" });
	state = jgpKanbanReducer(state, { type: "drop", targetColumnTitle: "Submitted" });
	...
	const submittedCard = resolveJgpKanbanColumns(state)
...
test("dropping into Submitted does not start a new generative lifecycle", async () => {
	...
	state = jgpKanbanReducer(state, { type: "drag-start", cardCode: "RFP-105", sourceColumnTitle: "RFP Intake" });
	state = jgpKanbanReducer(state, { type: "drop", targetColumnTitle: "Submitted" });
