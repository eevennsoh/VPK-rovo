...
export function QueueConversationWorkspace({
	...
	useLayoutEffect(() => {
		...
		const updateChatBodyShift = () => {
			...
			const overlap = Math.max(0, unshiftedChatRight - panelLeft + PANEL_CLEARANCE_PX);
			const availableLeftSpace = Math.max(0, unshiftedChatLeft - workspaceRect.left);
			const nextShift = -Math.round(Math.min(overlap, availableLeftSpace));
