...
function statusEvent(
	...
	return {
		...
		segments: [
			...
			statusLozengeSegment(from, JIRA_GOLDEN_JOURNEYS_V3_STATUS_PHASES),
			{ type: "transition-arrow" },
			statusLozengeSegment(to, JIRA_GOLDEN_JOURNEYS_V3_STATUS_PHASES),
		],
		createdAtMs,
	};
}
...
function reviewChecksForStep(step: JiraGoldenJourneysV3ReviewStep) {
	switch (step) {
		case "queued":
...
function reviewUpdatedAtMsForStep(step: JiraGoldenJourneysV3ReviewStep): number {
	switch (step) {
		case "queued":
...
export function resolveBuildStep(options: JiraGoldenJourneysV3StoryStateOptions): JiraGoldenJourneysV3BuildStep {
	return options.buildStep ?? "complete";
}

export function resolveFixStep(options: JiraGoldenJourneysV3StoryStateOptions): JiraGoldenJourneysV3FixStep {
