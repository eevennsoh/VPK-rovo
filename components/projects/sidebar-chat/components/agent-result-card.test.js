...
test("ChatPanel renders generated result cards after the turn container", () => {
	...
	assert.match(
		CHAT_PANEL_SOURCE,
		/renderTurnAfter=\{\(turn\) => \{[\s\S]*const restrictGeneratedAgentCardToMessageId = cards\?\.restrictGeneratedAgentCardToMessageId;[\s\S]*const generatedAgentResult =[\s\S]*isGeneratedAgentResult\(agentResult\) &&[\s\S]*hasTurnCompleteSignal\(message\) &&[\s\S]*\(restrictGeneratedAgentCardToMessageId === undefined \|\|[\s\S]*message\.id === restrictGeneratedAgentCardToMessageId\)[\s\S]*if \(artifactResult && !generatedAgentResult\)[\s\S]*if \(generatedAgentResult\)[\s\S]*className="w-full space-y-2" data-testid="rovo-generated-result-group"[\s\S]*<ArtifactResultCard[\s\S]*<AgentResultCard[\s\S]*onSelectAgent=\{handleAgentResultSelect\}/u,
	);
});
