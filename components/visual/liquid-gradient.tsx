...
function getCanvasPixelSize(canvas: HTMLCanvasElement): { width: number; height: number; dpr: number } {
...
export default function LiquidGradient({
	...
	contrast = 1.1,
	saturation = 1,
}: Readonly<LiquidGradientProps>): JSX.Element {
	...
	useEffect(() => {
		...
		gl.uniform1f(gl.getUniformLocation(program, "u_loop"), loop);
		...
		gl.uniform1f(gl.getUniformLocation(program, "u_distBias"), distBias);
		gl.uniform1f(gl.getUniformLocation(program, "u_jellify"), jellify ? 1.0 : 0.0);
		gl.uniform1f(gl.getUniformLocation(program, "u_ditherMode"), ditherMode);
		gl.uniform1f(gl.getUniformLocation(program, "u_dither"), dither);
		...
		for (let i = 0; i < MAX_SHADER_COLORS; i++) {
			...
			if (i < colors.length) {
				...
				gl.uniform4f(location, 0, 0, 0, 1.0);
			}
		}

		const start = performance.now();
		const render = () => {
			const { width, height, dpr } = getCanvasPixelSize(canvas);
			...
			gl.uniform1f(gl.getUniformLocation(program, "u_pixelRatio"), dpr);
