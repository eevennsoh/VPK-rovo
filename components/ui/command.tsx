...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-selected:bg-muted data-selected:text-foreground data-selected:*:[svg]:text-foreground data-[checked=true]:bg-bg-selected data-[checked=true]:text-text data-[checked=true]:data-selected:bg-bg-selected-hovered data-[checked=true]:data-selected:text-text data-[checked=true]:active:bg-bg-selected-pressed relative flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none in-data-[slot=dialog-content]:rounded-lg! [&_svg:not([class*='size-'])]:size-4 group/command-item data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      {...props}
    >
      ...
      {showCheckIcon ? (
        <CheckIcon
          aria-hidden={true}
          className="ml-auto self-center text-icon-subtle opacity-0 group-has-data-[slot=command-shortcut]/command-item:hidden group-data-[checked=true]/command-item:opacity-100 [&_svg]:text-icon-subtle!"
          size="small"
        />
      ) : null}
    </CommandPrimitive.Item>
