...
test("Tag front-slot demo uses shared logo treatment for 1P, 2P, and 3P marks", () => {
	...
	assert.match(TAG_DEMO_SOURCE, /import \{ AtlassianLogo, RovoColorIcon \} from "@\/components\/ui\/logo";/u);
	...
	assert.match(TAG_DEMO_SOURCE, /<IconTile[\s\S]*<AtlassianLogo name="atlassian" label="Atlassian" size="xxsmall" \/>[\s\S]*size="xxsmall"[\s\S]*variant="transparent"/u);
	...
	assert.match(logoMarkSource, /presentation\.hasBorder \? "size-2\.5" : "size-4"/u);
