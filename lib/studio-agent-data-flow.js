...
function appendUniqueString(items, seen, value) {
...
function normalizeStringList(value) {
	...
	for (const item of value) {
		appendUniqueString(items, seen, item);
	}
	return items;
}
	...
	for (const pattern of MEMORY_REFERENCE_PATTERNS) {
		...
		for (const match of source.matchAll(pattern)) {
			const label = getNonEmptyString(match[1])
				...
				.replace(/[.!?]+$/gu, "")
				.trim();
			appendUniqueString(references, seen, label);
		}
	}
...
function getAgentAutomationItems(config) {
	if (Array.isArray(config?.automationRules) && config.automationRules.length > 0) {
		return config.automationRules
			.map((rule, index) => {
				const name = getNonEmptyString(rule?.name) || `Automation ${index + 1}`;
				const eventCount = Array.isArray(rule?.triggers) ? rule.triggers.length : 0;
				return eventCount > 0
					? `${name} (${eventCount} event${eventCount === 1 ? "" : "s"})`
					: name;
			})
			.filter(Boolean);
	}
...
function buildAgentDataFlowMermaid(config = {}) {
	...
	const automations = getAgentAutomationItems(config);
	...
	buildSection(lines, "agent", "triggers", "Automations", automations, "runs when");
...
function buildAgentDataFlowRefinementPrompt({ config = {}, baselineMermaid = "" } = {}) {
	return [
		...
		"Use flowchart TD or graph TD. Keep labels short. Preserve concrete setup values.",
