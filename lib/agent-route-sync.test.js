...
test("withAgentParam preserves absolute URL origins", () => {
	assert.equal(
		withAgentParam("https://vpk.example/studio?foo=1#section", "acme-bot"),
		"https://vpk.example/studio?foo=1&agent=acme-bot#section",
	);
	assert.equal(
		withAgentParam("https://vpk.example/studio?foo=1&agent=acme-bot#section", null),
		"https://vpk.example/studio?foo=1#section",
	);
});
