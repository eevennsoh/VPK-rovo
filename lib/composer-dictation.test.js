...
test("restoreDictationBaseline resolves cancel text back to the pre-dictation snapshot", () => {
	assert.equal(restoreDictationBaseline("Existing draft"), "Existing draft");
	assert.equal(restoreDictationBaseline(""), "");
	assert.equal(restoreDictationBaseline(null), "");
	assert.equal(restoreDictationBaseline(undefined), "");
});
