...
test("auto-tagged mentions serialize with their source text instead of adding a sigil", () => {
	assert.equal(
		serialize.serializeComposerNode(
			doc(para(mention({ category: "app", label: "Slack", id: "slack", sourceText: "Slack" }))),
		),
		"Slack",
	);
	assert.equal(
		serialize.serializeComposerNode(
			doc(
				para(
					schema.text("Create an agent for "),
					mention({ category: "app", label: "Slack", id: "slack", sourceText: "slack" }),
					schema.text(" messages"),
				),
			),
		),
		"Create an agent for slack messages",
	);
});
