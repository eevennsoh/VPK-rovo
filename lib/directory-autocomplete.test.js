...
test("exact label matching finds unprefixed skill and tool labels with canonical casing", () => {
	const matches = autocomplete.getDirectoryAutocompleteExactLabelMatches({
		sources,
		text: "please create work item then CREATE GOOGLE DOC.",
	});

	assert.deepEqual(
		matches.map((match) => [match.from, match.to, match.traceFrom, match.traceTo, match.label, match.mention.category, match.trigger]),
		[
			[7, 23, 7, 23, "Create work item", "skill", "plain"],
			[29, 46, 29, 46, "Create Google Doc", "tool", "plain"],
		],
	);
});
