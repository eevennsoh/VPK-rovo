...
// in a family shares one brand color, so picking across the full set naturally
// randomizes both the imagery and the accent color.
...
/**
 * Picks a uniformly random avatar from the full set. Because each family shares
 * one brand color, this also randomizes the avatar's accent color.
 */
export function getRandomAgentAvatarSrc(): AgentAvatarSrc {
	const index = Math.floor(Math.random() * AGENT_AVATAR_SRCS.length);
	return AGENT_AVATAR_SRCS[index] ?? FALLBACK_AVATAR_SRC;
}
