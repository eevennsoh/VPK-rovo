...
// a random avatar from this pool when no explicit avatar is supplied. Avatars
// are grouped by agent family under `/avatar-agent/<group>/`, and every avatar
// in a family shares one brand color. The families are very different sizes
// (teamwork-agents alone holds ~44% of all avatars), so a flat uniform pick over
// files would land on the largest family's color (blue) far more often than the
// others. `getRandomAgentAvatarSrc` instead balances by color first (see below).
