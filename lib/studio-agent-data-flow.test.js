...
test("buildAgentDataFlowMermaid includes populated agent setup sections", () => {
	...
	assert.match(mermaid, /Automations/u);
...
test("buildAgentDataFlowMermaid normalizes malformed config payloads to the draft fallback", () => {
	assert.deepEqual(normalizeAgentDataFlowConfig(null), {});
	assert.deepEqual(normalizeAgentDataFlowConfig(["Tool"]), {});

	const mermaid = buildAgentDataFlowMermaid(null);

	assert.match(mermaid, /Untitled agent/u);
	assert.match(mermaid, /Draft configuration/u);
	assert.equal(isValidAgentDataFlowMermaid(mermaid), true);
});
